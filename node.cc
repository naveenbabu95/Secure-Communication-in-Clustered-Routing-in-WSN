...
#if defined(AES256)
uint8_t cipherlen1 = 32;
#elif defined(AES192)
uint8_t cipherlen1 = 24;
#elif defined(AES128)
uint8_t cipherlen1 = 16;
#endif
...
//int POSXS[10] = {137,26,145,22,23,128,66,131,150,117};
//int POSYS[10] = {126,71,200,45,131,121,77,28,5,84};
int POSXS[10] = { 15, 10, 20, 35, 40, 180, 150, 170, 150, 200 };
int POSYS[10] = { 20, 130, 80, 160, 110, 120, 20, 80, 140, 30 };

//int POSXS[5] = {137,26,145,22,23};
//int POSYS[5] = {126,71,200,45,131};
...
uint32_t Node::prng_rotate(uint32_t x, uint32_t k) {
    return (x << k) | (x >> (32 - k));
}

uint32_t Node::prng_next(void) {
...
void Node::prng_init(uint32_t seed) {
    uint32_t i;
...
//encryption - AES
void Node::printCiphers(uint8_t* str, uint8_t len)
{
    unsigned char i;
    for (i = 0; i < len; ++i) {
        printf("%.2x", str[i]);
...
string Node::plaintext_padding(string plaintext,uint8_t len) {
...
void Node::encrypt(string strplaintext,uint8_t key[], uint8_t cipher[]) {
    uint8_t len = cipherlen1;
    std::vector<uint8_t> myVector(strplaintext.begin(), strplaintext.end());
    uint8_t *plain_text = &myVector[0];
    //    phex(plain_text, len);
    EV<<"key is ";
    uint8_t key1[ECC_PRV_KEY_SIZE];
//    printCiphers(key, len);
    //    for(auto i=0;i<1;i++){
    //        EV<<key[i];
    //        key1[i] = key[i];
    //    }
...
string Node::decrypt(uint8_t* cipher,uint8_t key[], uint8_t len) {
    struct AES_ctx ctx;
    AES_init_ctx(&ctx, key);
...
double Node::reduceBattery(double distance, int sendOrReduce, int type = INFO,
        int packetCount = 1) {
    int localK;
    if (type == DATA)
        localK = (K * packetCount);
    if (type == INFO)
        localK = ((K / 4) * packetCount);
    double redu;
    if (sendOrReduce == SEND_REDUCE) {
        if (distance < THRS_DIST)
            redu = (E_ELEC * localK) + (E_FS * localK * pow(distance, 2));
        else
            redu = (E_ELEC * localK) + (E_MP * localK * pow(distance, 4));
    ...
    if (battery > (fullBattery * 0.80)) {
        ...
        getDisplayString().setTagArg("i2", 0, cs);
        ...
        getDisplayString().setTagArg("i", 0, ci);
    }
    return redu;
}
...
void Node::beginNextRound() {
    globalRound++;
    if (curRound == CLUSTER_DEFAULT_MEMBER - 1)
        curRound = 0;
    else
        curRound++;
...
void Node::updateDisplayStringPositionAndTag(int x = -1, int y = -1) {
    ...
    if (iAmHead)
        ...
        sprintf(chr, "MN"); //
    getDisplayString().setTagArg("t", 0, chr);
}
...
void Node::initialize() {
    noOfMessagesInARound = MessagesInARound;
    if (first_init) {
        battery = par("battery");
        fullBattery = par("battery");
        string a = par("registerationId");
        registerationId = a;
        globalRound = 1;
        ...
        receivedRoundCountVector.setName(
                "Received Round Count Vector IN NODE ");
        receivedRoundCountVector.record(globalRound);
        ...
        xPosition = POSXS[getNodeId()];
        yPosition = POSYS[getNodeId()];
        updateDisplayStringPositionAndTag(xPosition, yPosition);
        EV << xPosition << "," << yPosition << "\n"; // YZ proje iÃ§in

        if (xMax > yMax)
            networkRadius = xMax / 2.0;
        else
            networkRadius = yMax / 2.0;
    }

    clusterMembers = new int[NUM_NODES];
    ...
    clusterHead = server;
    //    EV<<"Psudorandom time "<<time(NULL)+getNodeId()+1;
    ////    srand(time(NULL)+getNodeId()+1); //as base and node[0] generating same public key
    //    srand(time(0));
    ...
    string result = registerationId;
    for (int i = 0; i < registerationId.size(); i++) {
        result[i] = registerationId[i] ^ str[i];
    }

    result.pop_back();
    string hash1 = sha256(result);

    Node2BaseMsg * msg3 = new Node2BaseMsg("SetupPhase");
    msg3->setSource(getNodeId());
    msg3->setPurpose("Election");
    msg3->setBattery(battery);
    const char * c = hash1.c_str();
    msg3->setHash(c);
    const char * c1 = str.c_str();
    msg3->setTimestamp(c1);
    for (int i = 0; i <= ECC_PUB_KEY_SIZE; i++)
        msg3->setPublickey(i, pub[i]);
    sendDirect(msg3, server, "in");*/

}

double Node::getDataForNode(int nodeId) {
    return (double)nodeId*10+noOfMessagesInARound;
}

void Node::handleMessage(cMessage *msg) {
    if (strcmp(msg->getName(), "bsinitmsg") == 0) {
        noOfMessagesInARound = MessagesInARound;
        bsInitbroadcastMsg * msg1 = check_and_cast<bsInitbroadcastMsg *>(msg);
        for (int i = 0; i < ECC_PUB_KEY_SIZE; i++) {
            basepub[i] = msg1->getPublicKey(i);
        }
        //        EV<<"Psudorandom time "<<time(NULL)+getNodeId()+1;
        //        EV<<"Length of "<<ECC_PRV_KEY_SIZE;
        //    srand(time(NULL)+getNodeId()+1); //as base and node[0] generating same public key
        srand(time(NULL)+getNodeId()+1);
        static int initialized = 0;
        if (!initialized) {
            prng_init(rand());
        }
        ...
        ecdh_generate_keys(pub, prv);
        ecdh_shared_secret(prv, basepub, sec);
        EV<<"\nsecret key of "<<getNodeId()<<" ";
        for (int i = 0; i < len; ++i) {
            EV<<sec[i];
            realsec[i]=sec[i];
        }
        ...
        result.pop_back();
        string hash1 = sha256(result);
//        EV<<"hash of nd"<<getNodeId()<<"is "<<hash1<<"\n";
        uint8_t temp[len];
        encrypt(hash1,realsec, temp);
        hash1 = "";
        hash1 = (char*)temp;
        hash1 = hash1.substr(0,len);
//        EV<<"Hash after encryption "<<hash1;
        Node2BaseMsg * msg3 = new Node2BaseMsg("SetupPhase");
        msg3->setSource(getNodeId());
        msg3->setPurpose("Election");
        msg3->setBattery(battery);
        ...
        for (int i = 0; i < ECC_PUB_KEY_SIZE; i++)
            msg3->setNodepublickey(i, pub[i]);
        sendDirect(msg3, server, "in");

    }
    if (strcmp(msg->getName(), "bcMElection") == 0) {
        EV << "Msg recieved from base\n";
        broadcastMsg * msg1 = check_and_cast<broadcastMsg *>(msg);
        int leader = msg1->getCHNodeNo();
        if (leader == getNodeId())
            iAmHead = true;
        else
            iAmHead = false;
        ...
        if (iAmHead) {
            ...
            getDisplayString().setTagArg("i", 0, ci);
            getDisplayString().setTagArg("i", 1, cc);
        } else {
            EV << getNodeId() << " --> CUR_ROUND : " << globalRound
                    << " <---------------------------------------------------\n";
            ...
            getDisplayString().setTagArg("i", 0, ci);
            getDisplayString().setTagArg("i", 1, "#BBBBBB");
        }
    ...
    if (!iAmDead) {
        /**/if (strcmp(msg->getName(), "bcM") == 0)
        {
            broadcastMsg * msg1 = check_and_cast<broadcastMsg *>(msg);
            broadcastArrived = true;
            int broX = msg1->getPosX();
            ...
            if (iAmHead) {
                EV << "(" << xPosition << ", " << yPosition << ") " << "nd["
                        << getNodeId() << "].head = " << str_f_path
                        << "=====already===ch=======\n";
                chDistance = MIN_DISTANCE;
...
//                    noOfMessagesInARound = MessagesInARound;
            ...
            //uncommented -5/6
            else if (chDistance == MIN_DISTANCE)
            {
        ...
        /**/if (strcmp(msg->getName(), "n2hdM") == 0)
        {
            node2headMsg *nmsg = check_and_cast<node2headMsg *>(msg);
            reduceBattery(
                    calculateDistance(xPosition, yPosition, nmsg->getPosX(),
                            nmsg->getPosY()), RECEIVE_REDUCE);
            if (iAmDead)
                return;
            if (nmsg->getDie() == true)
                dieCount++;
            ...
            std::string strtemp=nmsg->getTemperature();

            aggData.insert({nmsg->getSource(), strtemp});
            ...
            ch2headMsgStr.append(c);
            ch2headMsgStr.append("_");
        }

        /**/if (strcmp(msg->getName(), "n2hcM") == 0)
        {
            node2headMsg *nmsg = check_and_cast<node2headMsg *>(msg);
            reduceBattery(
                    calculateDistance(xPosition, yPosition, nmsg->getPosX(),
        ...
        /**/if (strcmp(msg->getName(), "wM") == 0)
        {
            ...
            string strtemperature = to_string(getDataForNode(getNodeId())); //converting double to string
            strtemperature = plaintext_padding(strtemperature, cipherlen1);
            encrypt(strtemperature,realsec, cipherText);
            string a = "";
            a = (char*)cipherText;
            a = a.substr(0,cipherlen1);
            msg2->setTemperature(a.c_str());
            msg2->setKind(0);
            reduceBattery(chDistance, SEND_REDUCE, DATA);
            msg2->setDie(iAmDead);
            EV << this->getNodeId()
                              << " --> Waiting Complete, Trying to send message to: "
                              << chFullPath << "\n";
            clusterHead = getModuleByPath(chFullPath);
...
//                    noOfMessagesInARound = MessagesInARound;
            ...
            //            }
            //            else if (!chooseSent && chDistance != MIN_DISTANCE) // Karar verilmiÅŸ, seÃ§tim mesajÄ± gÃ¶nderilecek
            //            {
            //                node2headMsg *msg2 = new node2headMsg("n2hcM");
            //                msg2->setSource(getNodeId());
            //                msg2->setPosX( xPosition );
            //                msg2->setPosY( yPosition );
            //                msg2->setKind(0); // KÄ±rmÄ±zÄ± nokta
            //                reduceBattery(chDistance, SEND_REDUCE, INFO);
            //                msg2->setBattery(battery);
            //                msg2->setDie(iAmDead);
            ...
            //                sendDirect(msg2, clusterHead, "in");
            //                chooseSent = true;
            //                if (iAmDead) // Mesaj gÃ¶nderdikten sonra Ã¶lmeyeceksem XXXXXXXXXXXXXXXXXXXXXXX
            //                {
            //                    getDisplayString().setTagArg("i", 0, "old/x_cross");
            //                }
            //                scheduleAt(simTime() + 1.0, waitEvent); // karar verilmiÅŸ, seÃ§tim mesajÄ± gÃ¶nderilmiÅŸ, yanlÄ±z dÃ¶nÃ¼ÅŸ gelmediÄŸi, ch nin Ã¶ldÃ¼ÄŸÃ¼ durumlar iÃ§in ne olur ne olmaz diye konuÅŸmuÅŸ schedule, etkisi son roundlarda belli oluyor.
            //            }
            ...
            //                sendDirect(msg2, server, "in"); // Sadece enerji dÃ¼ÅŸecek, baz mesaja tepki vermeyecek
            //                scheduleAt(simTime()+1.5, nextRoundEvent); // 4 saniye sonra yeni roundun mesajÄ± gÃ¶nderilecek
            //                if (!iAmDead) // Mesaj gÃ¶nderdikten sonra Ã¶lmeyeceksem
            //                {
        ...
        /**/if (strcmp(msg->getName(), "hwM") == 0)
        {
            if (!broadcastSent) {
                ...
                for (int i = 0; i < clustersize; i++) {
                    int val;
                    if (getNodeId() < clustersize)
                        val = i;
                    else
                        val = i + clustersize;
                    sprintf(chr, "LeachBasic.nd[%d]", val);
                ...
                broadcastSent = true;
                totalExpendedEnergy = reduceBattery(networkRadius * 2.82,
                        SEND_REDUCE, INFO);
                if (iAmDead)
                    return;
                scheduleAt(simTime() + 1.0, headWaitEvent);
            }
            if (littleBroadcastSent)
            {
                cancelEvent(nextRoundEvent);
                int serX = mainModule->par("bazX");
                int serY = mainModule->par("bazY");
                totalExpendedEnergy += reduceBattery(
                        calculateDistance(xPosition, yPosition, serX, serY),
                        SEND_REDUCE, DATA, clusterMemberCount);
                if (clusterMemberCount > 1)
                    reduceBattery(NO_DISTANCE, DATA_AGGREGATION_REDUCE, INFO,
                            clusterMemberCount);
                EV << this->getNodeId()
                                  << " --> Waiting Complete, SENDING ALL PACKETS!";
                head2baseMsg *msg2 = new head2baseMsg("h2bM");
                //                EV<<"length of AggData "<<aggData.size();
                msg2->setAggregatedData(aggData);
                bool flag=true;
                if(aggData.size() == 0)
                    flag = false;
                aggData.clear();
                ...
                if(iAmHead) {
                    noOfMessagesInARound--;
                    if(noOfMessagesInARound >= 0 ) {
                        scheduleAt(simTime() + 1.5, headWaitEvent);
                    }
                    else {
//                        noOfMessagesInARound = MessagesInARound;
                        scheduleAt(simTime(), nextRoundEvent);
                    }
                }
                if (!iAmDead) {
                    getDisplayString().setTagArg("i", 0, "misc/node_s");
                    ...
                    return;
                //                scheduleAt(simTime() + 3.0, nextRoundEvent);
            } else // broadcastSent && !littleBroadcastSent
            {
                ...
                for (int i = 0; i < clusterMemberCount; i++) {
                    dis = calculateDistance(xPosition, yPosition,
                            clusterMembersXs[i], clusterMembersYs[i]);
                    if (dis > maxdist) {
                        maxdist = dis;
                    }
                }
                ...
                clusterMemberCount++;
                clusterMembers[clusterMemberCount - 1] = getNodeId();
...
//                                                            << "   OOOOOOOOOO\n";
                ...
                for (int i = 0; i < clusterMemberCount; i++)
                    totalBattery += clusterMembersBats[i];
                double full_battery = par("battery").doubleValue();

                int final_index;
                if (totalBattery
                        < (full_battery * clusterMemberCount * percentageControl))
                    final_index = locational_index;
                else
                    final_index = battery_index;

                char * chrCh = new char[64];
                char * chr = new char[64];
                sprintf(chrCh, "LeachBasic.nd[%d]",
                        clusterMembers[final_index]);
        ...
        /**/if (strcmp(msg->getName(), "lbM") == 0)
        {
            cancelEvent(waitEvent);
            cancelEvent(headWaitEvent);
            littleBroMsg *nmsg = check_and_cast<littleBroMsg *>(msg);
            double chdis = calculateDistance(xPosition, yPosition,
                    nmsg->getPosX(), nmsg->getPosY());
